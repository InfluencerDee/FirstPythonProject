...
def calculate_slope (x1, x2, y1, y2):
    m = (y2 - y1) / (x2 - x1)
    return m
print(calculate_slope(10, 20, 10, 20))
...
def add_item (lst,item):
    lst.append(item)
    return lst
print(add_item([10, 20, 30, 40], 'potato'))

#Declare a function named remove_item. It takes a list and an item parameters. It returns a list with the item removed from it.
def remove_item (lst,item):
    lst.remove(item)
    return lst
print(remove_item([10, 20, 30, 40, 'potato'], 'potato'))
...
def evens_and_odds (number):
    ...
    for i in range(number + 1):
        if i % 2 == 0:
            ...
            odd_count += 1
    return "even numbers are : ", even_count, " odd numbers are: ", odd_count
print(evens_and_odds(10))

#Call your function factorial, it takes a whole number as a parameter and it return a factorial of the number
def factorial (number):
    if number < 0:
        return "Can't get factorial of a negative number"
    elif number == 0:
        return 1
    else:
        fac = 1
...
def is_empty(value):
    if isinstance(value, str):
        return "it is empty" if len(value) == 0 else "it is not empty"
    elif isinstance(value, (list, dict, tuple, set)):
        return "it is empty" if len(value) == 0 else "it is not empty"
    elif isinstance(value, int):
        return "integers don’t have emptiness in the usual sense"
    else:
...
def calculate_mode (*numbers):
    return mode(numbers)
print(calculate_mode(10, 10, 30, 40, 50))
...
def check_type(*items):
    ...
    for item in items:
        if type(item) != first_type:
            return "Items are not of the same type"
    return "All items are of type: ", first_type.__name__
print(check_type(10, 10, 30, 40, 50))
...
def check_var (variable):
    ...
    if variable.isidentifier() and not keyword.iskeyword(variable):
        ...
        return "Invalid Python variable name"
print(check_var("var"))
...
def most_spoken_languages(value, top_n=10):
    new_lang = []
    for item in value:
...
def most_populated_countries (country_list, top_n=10):
    ...
    for country in country_list:
        ...
        pop_list.append((name, population))


    sorted_pop_list = sorted(pop_list, key=lambda x: x[1], reverse=True)
    return sorted_pop_list[:top_n]
